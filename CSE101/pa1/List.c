...
bool equals(List A,
		List B){
	if(A==NULL || B==NULL){
		fprintf(stderr, "Error in calling equals(): both Lists are NULL!\n");
		exit(EXIT_FAILURE);
	}
...
List copyList(List L){
	...
	newlist->index=-1;
	newlist->length=0;
	Node* current_item=L->front;
	for(int i=0;i<(L->length);i++){
		append(newlist, current_item->data);
		current_item=current_item->next;
	}
	if(equals(L, newlist)==false){
		fprintf(stderr, "Could not create a copy of List L! New copy is not same as list L!\n");
		exit(EXIT_FAILURE);
	}
	return newlist;
}
